...
public partial class StormReplay
{
    ...
    public static StormReplayResult Parse(string fileName, ParseOptions? parseOptions = null)
    {
        ...
        // Parse() may be called repeatedly for live StormSave snapshots. Do not leak
        // the status/exception from a previous snapshot through the upstream statics.
        _stormReplayParseResult = StormReplayParseStatus.Incomplete;
        _failedReplayException = null;
    ...
    {
        MpqHeroesArchiveEntry entry = GetEntry(ReplayDetails.FileName, "save.details");
    ...
    {
        MpqHeroesArchiveEntry entry = GetEntry(ReplayInitData.FileName, "save.initData");
    ...
    {
        // Current StormSave snapshots do not contain a battlelobby member. Player
        // data needed by the live overlay is already present in details/initData.
        if (!_stormMpqArchive.TryGetEntry(ReplayServerBattlelobby.FileName, out MpqHeroesArchiveEntry? optionalEntry))
            return;
        MpqHeroesArchiveEntry entry = optionalEntry.Value;
    ...
    private MpqHeroesArchiveEntry GetEntry(string replayName, string saveName)
    {
        if (_stormMpqArchive.TryGetEntry(replayName, out MpqHeroesArchiveEntry? replayEntry))
            return replayEntry.Value;
        if (_stormMpqArchive.TryGetEntry(saveName, out MpqHeroesArchiveEntry? saveEntry))
            return saveEntry.Value;
        throw new FileNotFoundException($"Neither '{replayName}' nor '{saveName}' exists in the archive.");
    }
